...
Examples:
	...
	ProcessTypes  echo
	CreatedAt     2015-05-06 21:58:12.751741 +0000 UTC
	ENV[MY_VAR]   Hello World, this will be available in all process types.
`)
}
...
func runReleaseShow(args *docopt.Args, client *controller.Client) error {
	...
	listRec(w, "Artifact", fmt.Sprintf("%s+%s", artifact.Type, artifact.URI))
	listRec(w, "ProcessTypes", strings.Join(types, ","))
	listRec(w, "CreatedAt", release.CreatedAt)
